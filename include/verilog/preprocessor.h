...
extern verilog_default_net_type * verilog_new_default_net_type(
    unsigned int token_number,  //!< Token number of the directive.
    unsigned int line_number,   //!< Line number of the directive.
    ast_net_type type           //!< The net type.
);
...
19.6 of the spec.
*/
extern void verilog_preprocessor_resetall();
...
*/
extern verilog_include_directive * verilog_preprocessor_include(
    char * filename,        //<! The file to include.
    unsigned int lineNumber //!< The line number of the directive.
);
...
extern verilog_preprocessor_conditional_context * 
...
@param [in] lineno - The line the directive occurs on.
*/
extern void verilog_preprocessor_elseif(char * macro_name, unsigned int lineno);

/*!
@brief Handles an else statement being encountered.
@param [in] lineno - The line the directive occurs on.
*/
extern void verilog_preprocessor_else  (unsigned int lineno);

/*!
@brief Handles an else statement being encountered.
@param [in] lineno - The line the directive occurs on.
*/
extern void verilog_preprocessor_endif (unsigned int lineno);
...
@brief Tells the preprocessor we are now defining PLI modules and to tag
       them as such.
*/
extern void verilog_preproc_enter_cell_define(void);


/*!
@brief Tells the preprocessor we are no longer defining PLI modules.
*/
extern void verilog_preproc_exit_cell_define(void) ;
