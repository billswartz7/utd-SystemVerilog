...
{
    ast_node_attributes * tr = AST_CALLOC(1, ast_node_attributes);
    tr->attr_name   = name;
    tr->attr_value  = value;
    return tr;
}
...
ast_lvalue * ast_new_lvalue_id(ast_lvalue_type type, ast_identifier id)
{
    assert(type == NET_IDENTIFIER 
        ...
        || type == SPECPARAM_ID
        || type == PARAM_ID);
    ast_lvalue * tr = AST_CALLOC(1, ast_lvalue);
...
{
    ast_primary * tr = AST_CALLOC(1, ast_primary);
    ast_set_meta_info(&(tr->meta));
...
ast_expression * ast_new_expression_primary(ast_primary * p)
{
    ...
    ast_expression * tr = AST_CALLOC(1, ast_expression);
    ast_set_meta_info(&(tr->meta));
    assert(tr != NULL);
...
char * ast_expression_tostring(
    ...
    switch(exp -> type)
    {
        ...
        case MODULE_PATH_UNARY_EXPRESSION:
            ...
            tr = AST_CALLOC(strlen(pri)+5,char);
            strcat(tr,"(");
            strcat(tr, op); 
            strcat(tr,pri);
            strcat(tr,")");
...
ast_function_call * ast_new_function_call(ast_identifier  id,
                                          ...
                                          ast_list      * arguments)
{
    ast_function_call * tr = AST_CALLOC(1,ast_function_call);
    ast_set_meta_info(&(tr->meta));
...
ast_concatenation * ast_new_concatenation(ast_concatenation_type type,
                                          ...
                                          void * first_value)
{
    ast_concatenation * tr = AST_CALLOC(1,ast_concatenation);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_path_declaration * tr = AST_CALLOC(1,ast_path_declaration);
    ast_set_meta_info(&(tr->meta));
    ...
    )
{
    ast_simple_parallel_path_declaration * tr = 
            AST_CALLOC(1, ast_simple_parallel_path_declaration);
    ast_set_meta_info(&(tr->meta));
    ...
    )
{
    ast_simple_full_path_declaration * tr = 
            AST_CALLOC(1, ast_simple_full_path_declaration);
    ast_set_meta_info(&(tr->meta));
...
)
{
    ast_edge_sensitive_parallel_path_declaration * tr = 
        AST_CALLOC(1,ast_edge_sensitive_parallel_path_declaration);
    ast_set_meta_info(&(tr->meta));
...
)
{
    ast_edge_sensitive_full_path_declaration * tr = 
        AST_CALLOC(1,ast_edge_sensitive_full_path_declaration);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_task_enable_statement * tr = AST_CALLOC(1,ast_task_enable_statement);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_loop_statement * tr = AST_CALLOC(1,ast_loop_statement);
    ast_set_meta_info(&(tr->meta));
...
ast_case_item * ast_new_case_item(ast_list      * conditions,
                                  ast_statement * body)
{
    ast_case_item * tr = AST_CALLOC(1,ast_case_item);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_case_statement * tr = AST_CALLOC(1,ast_case_statement);
    ast_set_meta_info(&(tr->meta));
...
)
{
    ast_conditional_statement * tr = 
        AST_CALLOC(1,ast_conditional_statement);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_if_else * tr = AST_CALLOC(1, ast_if_else);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_wait_statement * tr = AST_CALLOC(1, ast_wait_statement);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_event_expression * tr = AST_CALLOC(1,ast_event_expression);
...
{
    ast_event_control * tr = AST_CALLOC(1,ast_event_control);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_delay_ctrl * tr = AST_CALLOC(1,ast_delay_ctrl);
    ast_set_meta_info(&(tr->meta));
...
)
{
    ast_timing_control_statement * tr = 
        AST_CALLOC(1,ast_timing_control_statement);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_single_assignment * tr = AST_CALLOC(1,ast_single_assignment);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_assignment * tr = AST_CALLOC(1,ast_assignment);
    ast_set_meta_info(&(tr->meta));

    tr -> type = ASSIGNMENT_HYBRID;
    tr -> hybrid = AST_CALLOC(1,ast_hybrid_assignment);
    tr -> hybrid -> type = type;
...
)
{
    ...
    tr -> procedural    =  AST_CALLOC(1,ast_procedural_assignment);
...
{
    ast_continuous_assignment * trc = AST_CALLOC(1, ast_continuous_assignment);
...
{
    ast_statement_block * tr = AST_CALLOC(1,ast_statement_block);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_disable_statement * tr = AST_CALLOC(1, ast_disable_statement);
    ast_set_meta_info(&(tr->meta));
    tr -> id = id;
    return tr;
}
...
{
    ast_statement * tr = AST_CALLOC(1,ast_statement);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_udp_port * tr = AST_CALLOC(1,ast_udp_port);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_udp_declaration * tr = AST_CALLOC(1,ast_udp_declaration);
    ast_set_meta_info(&(tr->meta));
...
ast_udp_instance * ast_new_udp_instance(
    ...
    ast_udp_instance * tr = AST_CALLOC(1,ast_udp_instance);
    ast_set_meta_info(&(tr->meta));
...
ast_udp_instantiation * ast_new_udp_instantiation(
    ...
    ast_udp_instantiation * tr = AST_CALLOC(1,ast_udp_instantiation);
    ast_set_meta_info(&(tr->meta));
...
ast_udp_initial_statement * ast_new_udp_initial_statement(
    ...
    ast_udp_initial_statement *tr=AST_CALLOC(1,ast_udp_initial_statement);
...
ast_udp_body * ast_new_udp_sequential_body(
    ...
    ast_udp_body * tr = AST_CALLOC(1,ast_udp_body);
    ast_set_meta_info(&(tr->meta));
...
ast_udp_combinatorial_entry * ast_new_udp_combinatoral_entry(
    ...
    ast_udp_combinatorial_entry * tr = 
        AST_CALLOC(1,ast_udp_combinatorial_entry);
    ast_set_meta_info(&(tr->meta));
...
ast_udp_sequential_entry * ast_new_udp_sequential_entry(
    ...
    ast_udp_sequential_entry * tr = AST_CALLOC(1,ast_udp_sequential_entry);
    ast_set_meta_info(&(tr->meta));
...
ast_generate_block * ast_new_generate_block(
    ...
    ast_generate_block * tr = AST_CALLOC(1,ast_generate_block);
    ast_set_meta_info(&(tr->meta));
...
ast_module_instantiation * ast_new_module_instantiation(
    ...
    ast_module_instantiation * tr = 
        AST_CALLOC(1,ast_module_instantiation);
    ast_set_meta_info(&(tr->meta));
...
ast_module_instance * ast_new_module_instance(
    ...
    ast_module_instance * tr = AST_CALLOC(1,ast_module_instance);
    ast_set_meta_info(&(tr->meta));
...
ast_port_connection * ast_new_named_port_connection(
    ...
    ast_port_connection * tr = AST_CALLOC(1,ast_port_connection);
    ast_set_meta_info(&(tr->meta));
...
ast_switch_gate * ast_new_switch_gate_d3(
    ...
    ast_switch_gate * tr = AST_CALLOC(1,ast_switch_gate);
    ast_set_meta_info(&(tr->meta));
...
ast_primitive_pull_strength * ast_new_primitive_pull_strength(
    ...
    ast_primitive_pull_strength * tr = 
        AST_CALLOC(1,ast_primitive_pull_strength);
    ast_set_meta_info(&(tr->meta));
...
ast_pull_gate_instance * ast_new_pull_gate_instance(
    ...
    ast_pull_gate_instance * tr = AST_CALLOC(1,ast_pull_gate_instance);
    ast_set_meta_info(&(tr->meta));
...
ast_pass_switch_instance * ast_new_pass_switch_instance(
    ...
    ast_pass_switch_instance * tr = 
                        AST_CALLOC(1,ast_pass_switch_instance);
    ast_set_meta_info(&(tr->meta));
...
ast_n_input_gate_instance * ast_new_n_input_gate_instance(
    ...
    ast_n_input_gate_instance * tr =
                        AST_CALLOC(1,ast_n_input_gate_instance);
...
ast_enable_gate_instance * ast_new_enable_gate_instance(
    ...
    ast_enable_gate_instance * tr = 
        AST_CALLOC(1,ast_enable_gate_instance);
    ast_set_meta_info(&(tr->meta));
...
ast_mos_switch_instance * ast_new_mos_switch_instance(
    ...
    ast_mos_switch_instance * tr = 
        AST_CALLOC(1,ast_mos_switch_instance);
    ast_set_meta_info(&(tr->meta));
...
ast_cmos_switch_instance * ast_new_cmos_switch_instance(
    ...
    ast_cmos_switch_instance * tr = AST_CALLOC(1,ast_cmos_switch_instance);
    ast_set_meta_info(&(tr->meta));
...
ast_pass_enable_switch * ast_new_pass_enable_switch(
    ...
    ast_pass_enable_switch * tr = AST_CALLOC(1,ast_pass_enable_switch);
    ast_set_meta_info(&(tr->meta));
...
ast_pass_enable_switches * ast_new_pass_enable_switches(
    ...
    ast_pass_enable_switches * tr = AST_CALLOC(1,ast_pass_enable_switches);
    ast_set_meta_info(&(tr->meta));
...
ast_n_input_gate_instances * ast_new_n_input_gate_instances(
    ...
    ast_n_input_gate_instances * tr = AST_CALLOC(1,ast_n_input_gate_instances);
    ast_set_meta_info(&(tr->meta));
...
ast_enable_gate_instances * ast_new_enable_gate_instances(
    ...
    ast_enable_gate_instances * tr = AST_CALLOC(1,ast_enable_gate_instances);
    ast_set_meta_info(&(tr->meta));
...
ast_n_output_gate_instance * ast_new_n_output_gate_instance(
    ...
    ast_n_output_gate_instance * tr = AST_CALLOC(1,ast_n_output_gate_instance);
...
ast_n_output_gate_instances * ast_new_n_output_gate_instances(
    ...
    ast_n_output_gate_instances * tr = AST_CALLOC(1,ast_n_output_gate_instances);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_switches * tr = AST_CALLOC(1,ast_switches);
    ast_set_meta_info(&(tr->meta));
...
ast_pull_strength * ast_new_pull_stregth(
    ...
    ast_pull_strength * tr =AST_CALLOC(1,ast_pull_strength);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_gate_instantiation * tr = AST_CALLOC(1,ast_gate_instantiation);
    ast_set_meta_info(&(tr->meta));
    tr -> type = type;
    return tr;
}
...
ast_parameter_declarations * ast_new_parameter_declarations(
    ...
    ast_parameter_declarations * tr = AST_CALLOC(1,ast_parameter_declarations);
    ast_set_meta_info(&(tr->meta));
...
ast_port_declaration * ast_new_port_declaration(
    ...
    ast_port_declaration * tr = AST_CALLOC(1,ast_port_declaration);
    ast_set_meta_info(&(tr->meta));
...
{
    ast_type_declaration * tr = AST_CALLOC(1,ast_type_declaration);
    ast_set_meta_info(&(tr->meta));
...
ast_list * ast_new_net_declaration(
    ...
    {
        ast_net_declaration * toadd =AST_CALLOC(1,ast_net_declaration);
...
ast_list * ast_new_reg_declaration(
    ...
    for (i = 0; i < type_dec -> identifiers -> items; i ++)
    {
        ast_reg_declaration * toadd =AST_CALLOC(1,ast_reg_declaration);
        toadd -> meta       = type_dec -> meta;
...
ast_list * ast_new_var_declaration(
    ...
    for (i = 0; i < type_dec -> identifiers -> items; i ++)
    {
        ast_var_declaration * toadd = AST_CALLOC(1,ast_var_declaration);
        toadd -> meta       = type_dec -> meta;
...
ast_delay_value * ast_new_delay_value(
    ...
    ast_delay_value * tr = AST_CALLOC(1,ast_delay_value);
    ast_set_meta_info(&(tr->meta));
...
ast_delay3 * ast_new_delay3(
    ...
    ast_delay3 * tr = AST_CALLOC(1,ast_delay3);
    ast_set_meta_info(&(tr->meta));
...
ast_delay2 * ast_new_delay2(
    ...
    ast_delay2 * tr = AST_CALLOC(1,ast_delay2);
    ast_set_meta_info(&(tr->meta));
...
ast_pulse_control_specparam * ast_new_pulse_control_specparam(
    ...
    ast_pulse_control_specparam * tr = AST_CALLOC(1,ast_pulse_control_specparam);
...
ast_range * ast_new_range(
    ...
    ast_range * tr = AST_CALLOC(1,ast_range);
...
{
    ast_range_or_type * tr = AST_CALLOC(1,ast_range_or_type);
    ast_set_meta_info(&(tr->meta));
...
ast_function_declaration * ast_new_function_declaration(
    ...
    ast_function_declaration * tr = AST_CALLOC( 1,ast_function_declaration);
    ast_set_meta_info(&(tr->meta));
...
ast_function_item_declaration * ast_new_function_item_declaration(){
    return AST_CALLOC(1,ast_function_item_declaration);
}
...
ast_task_port * ast_new_task_port(
    ...
    ast_task_port * tr = AST_CALLOC(1,ast_task_port);
    ast_set_meta_info(&(tr->meta));
...
ast_task_declaration * ast_new_task_declaration(
    ...
    ast_task_declaration * tr = AST_CALLOC(1,ast_task_declaration);
    ast_set_meta_info(&(tr->meta));
...
ast_block_reg_declaration * ast_new_block_reg_declaration(
    ...
    ast_block_reg_declaration * tr =
        AST_CALLOC(1,ast_block_reg_declaration);
...
ast_block_item_declaration * ast_new_block_item_declaration(
    ...
    ast_block_item_declaration * tr = 
        AST_CALLOC(1,ast_block_item_declaration);
    ast_set_meta_info(&(tr->meta));
...
ast_module_item * ast_new_module_item(
    ...
    ast_module_item * tr = AST_CALLOC(1,ast_module_item);
    ast_set_meta_info(&(tr->meta));
...
ast_module_declaration * ast_new_module_declaration(
    ...
    ast_module_declaration * tr = AST_CALLOC(1,ast_module_declaration);
    ast_set_meta_info(&(tr->meta));
...
ast_source_item * ast_new_source_item(ast_source_item_type type){
    ast_source_item * tr = AST_CALLOC(1,ast_source_item);
    ast_set_meta_info(&(tr->meta));
...
ast_identifier ast_new_identifier(
    ...
    ast_identifier tr = AST_CALLOC(1,struct ast_identifier_t);
    ast_set_meta_info(&(tr->meta));
...
ast_config_rule_statement * ast_new_config_rule_statement(
    ...
    ast_config_rule_statement * tr = AST_CALLOC(1,ast_config_rule_statement);
...
ast_config_declaration * ast_new_config_declaration(
    ...
    ast_config_declaration * tr = AST_CALLOC(1,ast_config_declaration);
    ast_set_meta_info(&(tr->meta));
...
ast_library_declaration * ast_new_library_declaration(
    ...
    ast_library_declaration * tr = AST_CALLOC(1,ast_library_declaration);
...
ast_library_descriptions * ast_new_library_description(
    ...
    ast_library_descriptions * tr = AST_CALLOC(1,ast_library_descriptions);
    ast_set_meta_info(&(tr->meta));
...
ast_number * ast_new_number(
    ...
    ast_number * tr = AST_CALLOC(1,ast_number);
    ast_set_meta_info(&(tr->meta));
...
{
    verilog_source_tree * tr = AST_CALLOC(1,verilog_source_tree);
