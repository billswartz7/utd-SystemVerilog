...
{
    ast_list * tr = AST_CALLOC(1, ast_list);
    tr -> head          = NULL;
    tr -> tail          = NULL;
    tr -> walker        = NULL;
    tr -> items         = 0;
...
void       ast_list_append(ast_list * list, void * data)
{
    if(list -> items == 0)
    {
        list -> head         = AST_CALLOC(1,ast_list_element);
        list -> head -> next = NULL;
        list -> head -> data = data;
    ...
    {
        list -> tail -> next = AST_CALLOC(1, ast_list_element);
...
void       ast_list_preappend(ast_list * list, void * data)
{
    ...
    {
        ast_list_element * to_add = AST_CALLOC(1, ast_list_element);
        to_add -> data = data;
...
ast_stack * ast_stack_new(){
    ast_stack * tr = AST_CALLOC(1,ast_stack);
    tr -> depth = 0;
    return tr;
}
...
void ast_stack_push(
    ...
    if(stack -> items == NULL)
    {
        stack -> items = AST_CALLOC(1,ast_stack_element);
        stack -> items -> data = item;
    } 
    else
    {
        ast_stack_element * toadd = AST_CALLOC(1,ast_stack_element);
        toadd -> data = item;
...
ast_hashtable * ast_hashtable_new(){
    ast_hashtable * tr = AST_CALLOC(1,ast_hashtable);
...
ast_hashtable_result ast_hashtable_insert(
    ...
    ast_hashtable_element * toinsert = AST_CALLOC(1,ast_hashtable_element);
    toinsert -> key = key;
    toinsert -> data = value;
